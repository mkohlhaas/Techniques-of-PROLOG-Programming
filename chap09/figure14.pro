% ESSLN0: An Expert System Shell With Logical Negation
% ====================================================================
% This file contains the original version of ESSLN, an expert system 
% shell that supports negative and quantified queries and subtypes, 
% which is written in Arity-Prolog 4.0 (the syntax of which is the
% same as Edinburgh-Prolog and it has some useful built-in predicates
% for environment and string handling as mentioned on page 365. As
% Arity-Prolog 5.1 and above do not allow automatic access to the 
% default world from anywhere as in Arity-Prolg 4.0, we have made
% ESSLN version 1, stored in the file \CHAP9\ESSLN.ARI, to be run with
% Arity-Prolog 5.1 or above. Also ESSLN version 2, which is stored in
% the file \CHAP9\ESSLN.STD, is written in standard Prolog and can be 
% run with most Prolog systems).
% To run the system in this file, take the following steps:
%
% a) Consult this file.
...
%    and in Vietnamese is 'liet-ke', etc.).
%
...
get_chars(1,_,[C|T],[],[C|T]) :- member(C,[32,61,58,44,46,63]).
...
convert(c,[[37|N]|P],VS,VS1,(A # C)) :- !,
     list_text(N,T),int_text(I,T),C is I/100,
...
convert_atom([[95|W],A|Rest],VS,VS1,[(T,_:_)|NP],
...
convert_atom([W|Rest],VS,VS1,[(T,_:_)|NP],
...
copy_clause([],[],[],[]) :- !.
copy_clause([(T,_:_)|P1],[(T,R:V)|P2],[R:V|VL],[type(R,T)|TL]) :- !,
     copy_clause(P1,P2,VL,TL).
...
write_goal_certainty(C) :-
     C =:= 1,!;
     C1 is C*100,float_text(C1,T,fixed(0)),int_text(C2,T),
     write_word('I am'),
...
% BUILTIN PREDICATES
% --------------------------------------------------------------------
builtin((predicate([Phrase,B],tvar_list(_,B)) :-
    trans_word('using the substitution',Phrase))).
builtin((predicate([Phrase|PNA],eval_non(A,_)) :-
    domain(A),
    (predicate(PNA,non(A)),!;
...
builtin((predicate([Phrase,L],instantiate(_,L,_)) :-
...
builtin((predicate([Phrase,L],instant(L,_)) :-
...
builtin((predicate([(_,A),In,(list,B)],member(A,B)) :- trans_word(in,In))).

builtin((domain(A) :- functor(A,F,N),(system(F/N);
...
prove(Goal,Depth,0,FailProof,_) :-
     collect_failproof(Goal,Depth,FailProof).

prove_branch(true,_,1,fact,_) :- !.
prove_branch(_,0,0,nonterminate,_) :- !.
...
prove_branch(A,_,C,(A # C :- system:R),_) :-
     sys(A),!,(A, C = 1, R = true; not(A), C = 0, R = false).
prove_branch(A,_,C,(A # C :- Proof),Rules) :-
     not(find_clause((A # C),_)),!,
...
recollect_body((B # C :- _),(B,C)) :- !.
recollect_body(((B # C :- _),Rest),((B,Bs),(C,Cs))) :-
...
display_proof_body((_:nottried,_)) :- !,
    write('...').
display_proof_body((B,C)) :-
    (C = 0,!,Q = some; Q = yes),
    copy(B,B1),
...
print_goal(call(A),Q) :- !,print_goal(A,Q).
print_goal(A,Q) :-
    domain(A),
    predicate(Phrase,A),!,
...
bind_var_char([_:X|Xs],Char,NextChar) :-
...
unrestricted([],_) :- !.
unrestricted([_:N|TVs],N) :- N1 is N+1,unrestricted(TVs,N1).
...
display_banner :-
    cls,banner,
    get0(C),(C = 101,cls,!,fail; true).

banner :-
    box(3,5,20,71),
    tmove(5,21),wc(8,196),wc(1,194),tmove(5,29),wc(1,191),
    vert(6,11,21),
    tmove(8,21),wc(5,196),wc(1,195),tmove(8,26),wc(1,180),
    tmove(11,21),wc(8,196),wc(1,193),tmove(11,29),wc(1,217),
    tmove(7,28),wc(7,196),wc(1,218),tmove(8,28),wc(1,179),
    tmove(9,28),wc(6,196),wc(1,192),tmove(9,34),wc(1,191),
    vert(10,12,34),tmove(12,25),wc(9,196),tmove(12,34),wc(1,217),
    tmove(8,35),wc(7,196),wc(1,218),tmove(9,35),wc(1,179),
    tmove(10,35),wc(6,196),wc(1,192),tmove(10,41),wc(1,191),
    vert(11,13,41),tmove(13,32),wc(9,196),tmove(13,41),wc(1,217),
    tmove(5,43),wc(1,194),vert(6,11,43),
    tmove(11,43),wc(6,196),wc(1,193),tmove(11,49),wc(1,217),
    tmove(6,50),wc(6,196),wc(1,194),tmove(6,56),wc(1,191),
    vert(7,11,50),vert(7,11,56),tmove(11,50),wc(1,193),
    tmove(11,56),wc(1,193),
    tmove(15,22),write('A MULTILINGUAL EXPERT SYSTEM SHELL'),
    tmove(16,23),write('THAT SUPPORTS QUANTIFIED QUERIES'),
    tmove(17,17),write('Copyright : Van Le, University of Canberra'),
    tmove(18,7),wc(63,196),
    tmove(19,7),write('Press any key to start, "e" to exit').

box(R,C,R1,C1) :-
    % RN is R1-R, 
    CN is C1-C, R2 is R+1,
    tmove(R,C),wc(CN,196),wc(1,218),
    tmove(R,C1),wc(1,191),
    vert(R2,R1,C),vert(R2,R1,C1),
    tmove(R1,C),wc(CN,196),wc(1,192),
    tmove(R1,C1),wc(1,217).
...
display_languages :-
    box(3,5,20,71),
    tmove(5,16),write('PLEASE CHOOSE A LANGUAGE FOR OUR CONVERSATION'),
    tmove(8,16),write('1. Algerian'),
    tmove(8,47),write(' 9. Philippino'),
    tmove(9,16),write('2. Dutch'),
    tmove(9,47),write('10. Portuguese'),
    tmove(10,16),write('3. English'),
    tmove(10,47),write('11. Romanian'),
    tmove(11,16),write('4. French'),
    tmove(11,47),write('12. Senegalese'),
    tmove(12,16),write('5. Indonesian'),
    tmove(12,47),write('13. Spanish'),
    tmove(13,16),write('6. Italian'),
    tmove(13,47),write('14. Swedish'),
    tmove(14,16),write('7. Moroccain'),
    tmove(14,47),write('15. Vietnamese'),
    tmove(15,16),write('8. Norwegian'),
    tmove(15,47),write('16. Other'),
    tmove(18,16),write('Your language of choice: ').

learn_new(_) :-
    tmove(18,16),write('This language is not available in this package.'),
    tmove(19,16),write('Please use English, French or Vietnamese.'),
