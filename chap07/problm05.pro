...
work(I,[1,_,B],[0,1,B],dust_room(I)).
work(I,[0,1,(_,P)],[0,0,(1,P)],vacuum_room(I)).
...
best_cost([Path|Rest],AnsPath) :-
    bcs_expand(Path,NPaths),
    merge(NPaths,Rest,NewList),
    best_cost(NewList,AnsPath).

bcs_expand([C,S|Path],NPaths) :-
    setof([C1,S1,S|Path],
          (extend([S|Path],S1),
...
% ------------------------------------------------
cost(_,(dust_room(_),_),6).
cost(_,(vacuum_room(_),_),8).
cost(_,(pickup_bin(_),_),3).
cost(_,(putdown_bin(_),_),1).
cost(_,(empty_bin(_),_),5).
cost(_,(move(I,J),_),C) :-
    I+J =:= 3,!, C = 4; C = 10.
