...
tobe_filled(your_course(X)).
tobe_filled(your_interest(X,L)).
tobe_filled(your_complete_units(L)).
...
display_goal(not(prerequisite_units(X,L))) :- !,
...
display_goal(member(X,L)) :- !,
    ...
    (length(L,N),N > 7,!,nl,tab(8); true), write(L).
display_goal(succeeds([(complete(X) :- B)|_])) :-
    write_list(['Unit ',X,' is not among your completed units']).
display_goal(succeeds([(prerequisite_units(X,_) :- B)|_])) :-
    write_list(['Unit ',X,' requires no prerequisites']).
...
prove(Goal,Depth,no,FailProof,Rules) :-
     collect_failbranches(Goal,FailProof).

prove_branch(true,D,yes,fact,Rules) :- !.
prove_branch(A,0,lost,toofar,Rules) :- !.
...
prove_branch(A,D,Result,(A :- system:Result),Rules) :-
...
prove_conj(yes,B,D,Result,Proof,Rules) :- !,
     prove_branch(B,D,Result,Proof,Rules).
prove_conj(RA, B,D,RA,B:nottried,Rules).
...
display_rule((A :- B)) :-
    member(B,[true,usergiven,nofact,system:R]),!,
    nl,write('  > Because'),
