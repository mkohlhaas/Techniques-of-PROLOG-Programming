...
% --------------------------------------------------------------------
update_states(_,ET,(ET,_,_,queue(_)),Messs,States,Messs,States) :- !.
update_states(_,_,(_,_,_,leaveq(_)),Messs,States,Messs,States) :- !.
update_states(_,ET,(ET,_,O,seton(_)),Messs,States,Messs,NStates) :- !,
    (member(free(O),States),!,NStates = States;
     NStates = [free(O)|States]).
update_states(_,ET,(ET,_,O,setoff(_)),Messs,States,Messs,NStates) :- !,
...
update_states(_,BT,(_,_,O,release(M)),Messs,States,
...
update_states(_,ET,(ET,_,O,waitfor(O1:M)),Messs,States,
...
% QUEUES PROCESSING
% -------------------------------------------------------------------
update_queue(CT,_,QName,Mess,I) :-
...
leave_queue(CT,(T,P,O,seize(A:_))) :- !,
    update_queue(CT,_,O,(P,T,A),-1).
leave_queue(_,_).
...
% ==============================================================
sim_cont(StopTime) :-
    state((CT,_,Rest)),simul_run((CT,StopTime,Rest)).
sim_cont(StopTime,snap) :-
    state((CT,_,Messs,States,Hist,Statis,_)),
...
linear_interpol(Table,P,X) :-
    append(_,[(Q,Y),(R,Z)|_],Table), Q =< P, P < R,!,
    X is Y + (Z - Y)*(P - Q)/(R - Q).
